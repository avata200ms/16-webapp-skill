---
description: Start the automated cycle for 16-fullstack-webapp
---

# Automated Workflow

Using orchestrator from: fullstack-webapp
...
**에이전트 팀** — 5명이 워크플로우(Workflow)를 통해 순차적으로 산출물을 파일로 릴레이하여 교차 검증한다.

## 에이전트 구성

| 에이전트 | 파일 | 역할 | 타입 |
|---------|------|------|------|
| architect | `../agents.md#@architect` | 요구사항, 아키텍처, DB, API 설계 | general-purpose |
| frontend-dev | `../agents.md#@frontend-dev` | React/Next.js 프론트엔드 구현 | general-purpose |
| backend-dev | `../agents.md#@backend-dev` | API, DB, 인증, 비즈니스 로직 | general-purpose |
| qa-engineer | `../agents.md#@qa-engineer` | 테스트 전략, 테스트 코드, 코드 리뷰 | general-purpose |
| devops-engineer | `../agents.md#@devops-engineer` | CI/CD, 인프라, 배포, 모니터링 | general-purpose |
